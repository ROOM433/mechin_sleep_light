...
- 부저 (알람용)
- LED (상태 표시용)
- AC 디머 모듈(트라이악, 제로크로스 출력/게이트 입력)
- E26 디밍 지원 LED 전구(예: NASPIL 7W, 220V/60Hz, TRIAC 디밍 지원)
- 점퍼 와이어
- 라즈베리파이 (서버용)
...
ESP32    AC 디머 (예: RobotDyn AC Light Dimmer)
------   -------------------------------------
27    -> ZC (제로크로스 출력)
25    -> DIM (게이트 트리거 입력)
GND   -> GND (저전압 공통)
...
AC 메인(220V/60Hz)
------
L(상)  -> 디머 L IN -> 디머 L OUT -> 전구 L
N(중성) -> 전구 N (직결)
```

주의: 220V 배선 작업은 감전/화재 위험이 있습니다. 절연 케이스, 퓨즈(250V 0.25~0.5A), 차단기, 접지 및 KC 인증 부품을 사용하세요. 전구는 반드시 “디밍 지원(TRIAC 호환)” 모델을 사용해야 합니다.
