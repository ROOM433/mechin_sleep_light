...
#define WIFI_SSID "iptime2.4G083"
#define WIFI_PASSWORD "81234567"
#define SERVER_HOST "192.168.2.3"  // 백엔드 서버 IP 주소
...
public:
  ...
  sleep_data read_sleep_data() {
    ...
    // 움직임 점수 계산
    float accel_mag = sqrt(data.accel_x * data.accel_x + data.accel_y * data.accel_y + data.accel_z * data.accel_z);
    ...
    // 수면 단계 판단
    if (data.movement_score < 0.06) {
      data.sleep_stage = 2;  // 깊은잠
    } else if (data.movement_score < 0.2) {
...
public:
  ...
  sleep_data read_sleep_data() {
    ...
    // Yaw: Z축 회전 (자기계 사용)
    float mag_x_cal = data.mag_x * cos(data.pitch * 0.0174533) + data.mag_z * sin(data.pitch * 0.0174533);
    float mag_y_cal = data.mag_x * sin(data.roll * 0.0174533) * sin(data.pitch * 0.0174533) + data.mag_y * cos(data.roll * 0.0174533) - data.mag_z * sin(data.roll * 0.0174533) * cos(data.pitch * 0.0174533);
...
inline int clampBright(int v) {
  return constrain(v, MIN_BRIGHT, 100);
}
inline void setPowerClampedDirect(int v) {
  dimmer.setPower(clampBright(v));
}

// ===== Dimming engine state =====
enum { PATTERN_SMOOTH = 1,
       PATTERN_STEP = 2,
       PATTERN_PULSE = 3,
       PATTERN_SAW = 4 };

struct DimState {
  uint8_t pattern = 0;           // 1~4 (PATTERN_*)
  uint8_t maxBright = 100;       // MIN_BRIGHT~100
  bool running = false;          // 패턴 동작 여부
  unsigned long startTime = 0;   // 패턴 시작 시각 (millis)
  unsigned long cycleMs = 4000;  // 한 사이클 시간(ms)
} dimst;
...
public:
   ...
   * @param intervalOverride 전체 사이클 시간(ms). 0이면 기본값(4000ms)
   */
  void bulbDimming(uint8_t pattern, uint8_t maxBright, uint16_t intervalOverride = 0) {
    dimst.pattern = constrain(pattern, PATTERN_SMOOTH, PATTERN_SAW);
    dimst.maxBright = (uint8_t)max((int)MIN_BRIGHT, (int)constrain(maxBright, 10, 100));
    dimst.cycleMs = (intervalOverride > 0) ? (unsigned long)intervalOverride : 4000UL;
    dimst.startTime = millis();
    dimst.running = true;
    ...
    char msg[160];
    snprintf(msg, sizeof(msg),
             "[DIMMER] Pattern %d started, maxBright:%d, cycle:%lu ms",
             dimst.pattern, dimst.maxBright, dimst.cycleMs);
    st_debug_print(2, msg);
  }
  ...
  void setPowerClamped(int level) {
    dimst.running = false;   // 패턴 중지
    sunrise_active = false;  // 선라이즈 중지

    int clamped = clampBright(level);
    setPowerClampedDirect(clamped);
  ...
  void updateDimming() {
    ...
    // 1) 선라이즈 우선 처리
    if (sunrise_active) {
      unsigned long elapsed = now - sunrise_start_time;
      if (elapsed >= sunrise_duration) {
        ...
        new_level = clampBright(new_level);
    ...
    // 2) 패턴이 아니면 아무것도 안 함
    if (!dimst.running || dimst.cycleMs == 0) return;

    // 3) 현재 페이즈(0~1) 계산
    unsigned long elapsed = now - dimst.startTime;
    float phase = (float)(elapsed % dimst.cycleMs) / (float)dimst.cycleMs;  // 0.0 ~ 1.0

    int minB = (int)MIN_BRIGHT;
    int maxB = (int)dimst.maxBright;
    int range = max(1, maxB - minB);
    float bf = (float)minB;  // 계산용 float brightness
    ...
    switch (dimst.pattern) {
      case PATTERN_SMOOTH:
        {
          ...
          float t;
          if (phase < 0.5f) {
            t = phase / 0.5f;  // 0~0.5 → 0~1
          } else {
            t = (1.0f - phase) / 0.5f;  // 0.5~1 → 1~0
          }
          bf = (float)minB + t * range;
          break;
        }

      case PATTERN_STEP:
        {
          // 먼저 SMOOTH 삼각파를 만들고 → 4단계로 양자화
          float t;
          if (phase < 0.5f) {
            t = phase / 0.5f;
          } else {
          ...
          float tri = (float)minB + t * range;  //  연속 값

          const int steps = 4;                       // 4단계 스텝
          float norm = (tri - minB) / (float)range;  // 0~1
          if (norm < 0.0f) norm = 0.0f;
          if (norm > 1.0f) norm = 1.0f;
          int idx = (int)(norm * (steps - 1) + 0.5f);  // 0~steps-1

          bf = (float)minB + (range * idx) / (float)(steps - 1);
          break;
        }

      case PATTERN_PULSE:
        {
          // 페이드업(30%) → 최대 유지(20%) → 페이드다운(30%) → 최소 유지(20%)
          const float upDur = 0.3f;
          const float holdHi = 0.2f;
          const float downDur = 0.3f;
          const float upEnd = upDur;
          const float holdHiEnd = upDur + holdHi;
          const float downEnd = upDur + holdHi + downDur;

          if (phase < upEnd) {
            // fade up
            float t = phase / upDur;
            bf = (float)minB + t * range;
          } else if (phase < holdHiEnd) {
            // hold max
            bf = (float)maxB;
          } else if (phase < downEnd) {
            // fade down
            float t = (phase - holdHiEnd) / downDur;
            bf = (float)maxB - t * range;
          } else {
            // hold min
            bf = (float)minB;
          }
      ...
      default:
        {
          // 톱니파: MIN → MAX → 즉시 MIN
          bf = (float)minB + phase * range;
          break;
        }
    }

    // 4) 최종 밝기 정수로 변환 + 클램프
    int level = clampBright((int)(bf + 0.5f));  // 반올림
...
private:
  ...
  void handleServerMessage(const char *message) {
    ...
    if (command == "start_monitoring") {
      ...
      // 서버: { "command": "set_alarm", "delay_ms": ... }
      unsigned long delayMs = doc["delay_ms"] | 0;

      if (delayMs == 0) {
        st_debug_print(2, "[ALARM] set_alarm received but delay_ms is 0");
      } else {
        setAlarm(delayMs);
      }
    }
      ...
      uint16_t intervalMs = doc["interval_ms"] | 0;
      dimmer.bulbDimming(pattern, maxBright, intervalMs);
  ...
  void setAlarm(unsigned long delayMs) {
    // 현재 millis() 기준으로 알람 목표 시각 설정
    alarm_time = millis() + delayMs;
    alarm_active = true;
    alarm_triggered_notified = false;  // 알람 알림 상태 리셋

    char msg[80];
    snprintf(msg, sizeof(msg),
             "[ALARM] Alarm set: delay %lu ms (target millis=%lu)",
             delayMs, alarm_time);
    st_debug_print(2, msg);
  }
